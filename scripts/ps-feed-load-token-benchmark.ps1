param(
  ...
  [int]$DurationSec = 15,
  [int]$TimeoutSec = 10,
  [string]$Label = "ps-feed-token-benchmark",
  [string]$Output = ""
)
...
foreach ($level in $concurrencyLevels) {
  Write-Host ("Running token-feed level {0} for {1}s" -f $level, $DurationSec)

  $jobs = @()
  for ($i = 0; $i -lt $level; $i++) {
    $jobs += Start-Job -ScriptBlock {
      param($RootUrl, $CookieDomain, $Token, $ReqTimeoutSec, $DurSec)
      ...
      $cookie = New-Object System.Net.Cookie('authjs.session-token', $Token, '/', $CookieDomain)
    ...
    } -ArgumentList $BaseUrl, $domain, $SessionToken, $TimeoutSec, $DurationSec
  }
