...
export default function ToplulukPage() {
  ...
  const [typeFilter, setTypeFilter] = useState<DiscoveryType>("");
  ...
  useEffect(() => {
    fetch("/api/sports")
      .then((r) => r.json())
      .then((d) => setSports(d.sports ?? d.data ?? []))
      .catch(() => {});
  ...
  const handleJoin = async (id: string) => {
    ...
    try {
      ...
      setCommunities((prev) =>
        prev.map((community) =>
          community.id === id ? { ...community, myStatus: status as "PENDING" | "APPROVED" } : community
        )
      );
    } catch (error: unknown) {
  ...
  const hasResults = communities.length > 0;
        ...
        {loading ? (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {Array.from({ length: 6 }).map((_, idx) => (
              <div key={idx} className="h-48 rounded-2xl bg-gray-200 dark:bg-gray-800 animate-pulse" />
            ))}
          </div>
        ) : !hasResults ? (
                ...
                <div className="grid grid-cols-3 gap-2">
                  {([
                    ...
                    <button
                      key={typeItem.value}
                      type="button"
                      onClick={() => setForm((prev) => ({ ...prev, type: typeItem.value as CreateType, isPrivate: typeItem.value === "CLUB" ? true : prev.isPrivate }))}
                      className={`relative flex flex-col items-center gap-1.5 p-4 rounded-xl border-2 text-center transition-all ${
                        form.type === typeItem.value
                    ...
                    >
                      ...
                      <span className="text-2xl">{typeItem.icon}</span>
                      <span className="text-sm font-semibold text-gray-800 dark:text-gray-200">{typeItem.label}</span>
                      <span className={`text-[10px] font-medium px-1.5 py-0.5 rounded-full ${typeItem.badgeColor}`}>{typeItem.badge}</span>
                    </button>
                  ))}
                </div>
              </div>
