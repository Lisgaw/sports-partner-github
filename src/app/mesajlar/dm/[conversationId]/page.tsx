...
export default function DmChatPage({ params }: { params: Promise<{ conversationId: string }> }) {
  ...
  const loadMessages = useCallback(async () => {
    try {
      ...
      if (res.success && res.data) {
        ...
        // Derive partner from first message where sender != me
        if (!partner && res.data.messages.length > 0) {
          const myId = session?.user?.id;
          const otherMsg = res.data.messages.find((m) => m.senderId !== myId);
  ...
  }, [conversationId, partner, session?.user?.id]);
