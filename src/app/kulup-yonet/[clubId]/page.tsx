...
export default function ClubManagePage() {
  ...
  const fetchClub = useCallback(async () => {
    try {
      ...
      toast.error("Kulüp bilgisi yüklenemedi");
    }
  }, [clubId, router]);
  ...
  const fetchMembers = useCallback(async () => {
    try {
      const [pendingRes, approvedRes] = await Promise.all([
        fetch(`/api/clubs/${clubId}/members?status=PENDING`),
        fetch(`/api/clubs/${clubId}/members?status=APPROVED`),
      ]);
  ...
  }, [authStatus, fetchClub, fetchMembers, router]);

  // Check captain access after data loaded
  useEffect(() => {
    if (!loadingData && session && approved.length > 0) {
      const myCaptain = approved.find(
  ...
  const tabs: { key: Tab; label: string; count?: number }[] = [
    ...
    { key: "settings", label: "Kulüp Ayarları" },
  ];
      ...
      <div className="max-w-3xl mx-auto px-4">
        ...
        <div className="mb-6">
          ...
          <h1 className="text-2xl font-bold">
            {club?.name ?? "Kulüp Yönetimi"}
          </h1>
          <p className="text-gray-400 text-sm mt-1">
            {club?.sport?.name} · {club?.city?.name}
            {club?.isPrivate && (
            ...
            {approved.length === 0 ? (
              ...
              approved.map((m) => {
                ...
                const actions = isSelf
                  ...
                  : [
                      { label: "↑ Kaptan Yap", action: "promote", style: "blue" as const },
            ...
            <div className="flex items-center gap-3">
              <button
                ...
                className={`relative w-11 h-6 rounded-full transition-colors ${
                  form.isPrivate ? "bg-indigo-600" : "bg-gray-700"
                }`}
              >
                <span
              ...
              <span className="text-sm text-gray-300">
                Özel kulüp (yeni üyeler onay bekler)
              </span>
            </div>
            <button
              type="submit"
